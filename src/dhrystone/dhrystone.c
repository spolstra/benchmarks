...
/*#define GTIMEOD               /* Use gettimeofday(3) time function */
...
void Proc0(void)
{
	...
	gettimeofday(&tv, 0); starttime = tv.tv_sec * 1000000 + tv.tv_usec;
	for (i = 0; i < LOOPS; ++i);
        gettimeofday(&tv, 0);
	nulltime = tv.tv_sec * 1000000 + tv.tv_usec - starttime; /* Computes o'head of loop */
#endif
#ifdef TIMES
	time_t			starttime;
	time_t			benchtime;
...
#ifdef GTIMEOD
        gettimeofday(&tv, 0);
	benchtime = tv.tv_sec * 1000000 + tv.tv_usec - starttime - nulltime;
	printf("Dhrystone(%s) time for %ld passes = %ld (microseconds)\n",
		Version,
		(long) LOOPS, benchtime);
	printf("This machine benchmarks at %ld dhrystones/second\n",
               (((long) LOOPS) * 1000000UL / benchtime));
#endif
#ifdef TIME
